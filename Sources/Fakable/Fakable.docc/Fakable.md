# ``Fakable``

Generate `fake()` test fixtures for your structs and enums.

## Overview

`Fakable` provides a single macro, ``Fakable(condition:)``, that adds a `fake()` factory
method to the type it is attached to. Every parameter has a default derived from
its type, so a test names only the values it actually cares about:

```swift
import Fakable

@Fakable
struct Item {
    let itemId: String
    let name: String?
    let price: Int
}

let item = Item.fake(itemId: "item-1", price: 500)
// name defaults to nil, and the other values to "" and 0
```

The generated method is wrapped in `#if DEBUG` by default, so it never reaches a
release build. ``FakeCompilationCondition`` selects a different guard, or none.

## Topics

### Essentials

- <doc:GettingStarted>
- ``Fakable(condition:)``
- ``FakeCompilationCondition``

### Going further

- <doc:AdvancedUsage>
